...
\usepackage{url,natbib,graphicx,parskip}
...
Furthermore, we make available an R package \textit{dobin} \citep{dobin}, which computes the DOBIN basis. In addition, all examples in this paper are available in the supplementary materials at \textit{https://github.com/sevvandi/Outlier-Basis}.  We start the next section  with the mathematical framework of DOBIN. % in Section~\ref{sec:MathFrame}.
...
\begin{algorithm}\fontsize{9}{10}\selectfont
	...
	\Output{~The space $Y$ consisting of all $\{\bm{y}_l \}_{l=1}^M$ and the associated indices $i$ and $j$.}
...
\begin{enumerate}
    ...
    \item Each $\bm{y}_l \in Y$ is constructed from two points $\bm{x}_i$ and $\bm{x}_j$ in $X$ with $T(l) = (i,j)$, where $T$ is a list consisting of the pairs in $X$ that are used to construct $Y$. In addition, $\bm{x}_i$ and $\bm{x}_j$ are $k$-nearest neighbours for some $k \in \{k_1, \ldots, k_2\}$.
    \item For $i,j,l$ as in above,  $\bm{y}_l = \left(\bm{x}_i - \bm{x}_j \right) \circ \left(\bm{x}_i - \bm{x}_j \right) $, where $\circ$ denotes the element-wise vector product. 
    \item The $Y$ space contains points $\bm{y}_l$ with relatively high $\sum_{m=1}^p y_{lm}$, i.e. it contains vectors that contribute to high knn distances in the $X$ space.  
\end{enumerate}

\subsection{Maximising the distance between points}\label{sec:MathFrame2}
%The $Y$ space has points $\bm{y}_l$ that correspond to neighbouring points $\bm{x}_i$ and $\bm{x}_j$ in $X$, which have relatively high $k$ nearest neighbour distances. 
...
\end{align}
with $\bm{y}_{l_0} = \bm{y}_{l}$. The set $\Theta = \left\{ \eta_1, \eta_2, \ldots , \eta_p  \right\}$ gives a basis for $Y$ with each $\eta_{i+1}$ maximising $\sum_{l}\left\langle \eta\, ,  \bm{y}_{l_i} \right\rangle$. %which aims to maximise the quantity $\sum_{l}\left\langle \eta\, ,  \bm{y}_{l} \right\rangle$.  
...
\begin{figure*}[!h]
	\centering
	\subfloat[][]{
		\includegraphics[scale=0.48]{LOF_Nemenyi_1.pdf}
		\label{fig:LOF_FAT1}
	}%
	\subfloat[][]{
		\includegraphics[scale=0.48]{LOF_Density_1.pdf}
		\label{fig:LOF_FAT2}
	}%
...
\begin{figure*}[!h]
	\centering
	\subfloat[][]{
		\includegraphics[scale=0.48]{KNN_Nemenyi_1.pdf}
		\label{fig:KNN_FAT1}
	}%
	\subfloat[][]{
		\includegraphics[scale=0.48]{KNN_Density_1.pdf}
		\label{fig:KNN_FAT2}
	}%
...
\begin{figure*}[!h]
	\centering
	\subfloat[][]{
		\includegraphics[scale=0.48]{IFOREST_Nemenyi_1.pdf}
		\label{fig:IFOREST_FAT1}
	}%
	\subfloat[][]{
		\includegraphics[scale=0.48]{IFOREST_Density_1.pdf}
